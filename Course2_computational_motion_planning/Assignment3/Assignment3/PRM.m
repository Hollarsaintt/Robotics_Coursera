...
for i = 2:nsamples
    ...
    k_NN = samples(:,ind(1:k)); % k nearest samples to the new sample.
    for nb = 1:k,
        val = LocalPlanner(k_NN(:,nb), samples(:,i));
        
        if(val),
            nedges = nedges + 1; % update number of edges.
            edges(nedges,:) = [ind(nb), i]; % add the index in samples of the samples that just form a new edge.
            edge_lengths(nedges) = Dist(x, samples(:,ind(nb))); % find the distance between the new sample and the sample it forge path with.   
        end
    end
